...
async def receive_message(request: Request):
    ...
    try:
        ...
        # SALUDO
        # =====================
        if any(palabra in text for palabra in ["hola", "buenas", "hey", "saludos"]):
            send_message(
                phone,
                "Hola 😊 qué gusto saludarte.\n"
                "Cuéntame, ¿estás buscando un *carro* o un *camión*?"
            )
            return "EVENT_RECEIVED"
        ...
        if text in ["sí", "si", "claro", "de una", "ok", "vale"]:
            send_message(
                phone,
                "Genial 🙌 ¿En qué horario te queda mejor la llamada?. Contamos con horario de mañana y tarde."
            )
            return "EVENT_RECEIVED"
        ...
        # =====================
        if any(palabra in text for palabra in ["mañana", "tarde"]):
            guardar_lead(
